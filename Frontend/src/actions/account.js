...
export const updateDefaultAccount = async (accountId) => {
  try {
    const response = await fetch(
      `${backendUrl}/api/accounts/set-default/${accountId}`,
      {
        method: "PUT",
        credentials: "include",
      }
...
export const getAccountWithTransactions = async (accountId) => {
  try {
    const response = await fetch(
      `${backendUrl}/api/accounts/${accountId}/transactions`,
      {
        method: "GET",
        credentials: "include",
      }
...
export const handleBulkDelete = async (transactionIds) => {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/transactions/delete`, {
      method: "POST",
      headers: {
